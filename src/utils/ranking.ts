...
/*
  Get top level comments ranked comment
 */
function getTopComments(comments: Comment[], topLevelLimit: number): RankedComment[] {
  const rankedComments = rankComments(comments);
  const limitedTopLevel = (topLevelLimit && topLevelLimit > 0) ? rankedComments.slice(0, topLevelLimit) : rankedComments;
  return limitedTopLevel;
}
